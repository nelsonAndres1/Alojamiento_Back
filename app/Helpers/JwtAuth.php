...
class JwtAuth
{
    ...
    public function getCategoria($docemp)
    {
        $categoria = Subsi15::where('cedtra', $docemp)->Where('estado', 'A')->get();
        if (sizeof($categoria) > 0) {
            foreach ($categoria as $ca) {
                $token = array(
                    ...
                    'cedtra' => $ca->cedtra,
                    'coddoc' => $ca->coddoc,
                    'apellidos' => trim($ca->priape).' '.trim($ca->segape),
                    'nombres' => trim($ca->prinom).' '.trim($ca->segnom),
